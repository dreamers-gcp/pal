...
import type { CalendarRequest, Classroom, FacilityBooking } from "../../types";
...
export function AdminCalendarScreen() {
  ...
  const load = useCallback(async (silent?: boolean) => {
    ...
    const [b, f, rooms] = await Promise.all([
  ...
  const sections = useMemo(() => {
    if (scheduleScope === "week") {
      return buildAgendaSectionsForInterval(weekStart, weekEnd, bookings, facility, []);
    }
    if (scheduleScope === "day") {
      const d0 = startOfDay(cursor);
      return buildAgendaSectionsForInterval(d0, endOfDay(cursor), bookings, facility, []);
    }
    return buildAgendaSectionsForMonth(visibleMonth, bookings, facility, []);
  }, [scheduleScope, weekStart, weekEnd, cursor, visibleMonth, bookings, facility]);
  ...
  }, [scheduleScope, cursor, weekStart, bookings, facility]);
  ...
  const loading = loadingBookings || loadingFacility || loadingClassrooms;

  const empty =
    scheduleScope === "week"
      ? "No classes or bookings this week in the loaded range."
      : scheduleScope === "day"
