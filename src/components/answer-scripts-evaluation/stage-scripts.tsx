...
import { fallbackMetaFromFileName, metaFromExtractScriptApiBody } from "@/lib/extract-exam-script-meta";
    ...
    for (const { id, file } of jobs) {
      void (async () => {
        ...
        try {
          ...
          const meta = metaFromExtractScriptApiBody(res.ok ? raw : null);

          const pages = meta.pages && meta.pages > 0 ? meta.pages : 1;
          const nameFromPdf = meta.name.trim();
          const rollFromPdf = meta.rollNo.trim();
  ...
  return (
    <div className="space-y-6">
      <Card className="text-base">
        <CardHeader>
          <CardTitle className="text-xl">Student answer scripts</CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-6">
          ...
          >
            ...
            <p className="text-base font-medium">Drop multiple script PDFs here</p>
            <p className="mt-1 text-sm text-muted-foreground">or click to browse</p>
          </label>

          <div className="rounded-lg border bg-[#01696f]/[0.06] px-4 py-3.5 text-base">
          ...
          <div className="overflow-x-auto rounded-xl border">
            <table className="w-full min-w-[720px] text-base">
              ...
              <tbody>
                {scripts.length === 0 ? (
                    <tr>
                    <td colSpan={5} className="px-3 py-10 text-center text-base text-muted-foreground">
                  ...
                  scripts.map((r) => (
                    <tr key={r.id} className="border-b border-border/60 last:border-0">
                      <td className="max-w-[200px] truncate px-3 py-2.5 font-mono text-sm">
                        {r.fileName}
                      </td>
                      ...
                      <td className="px-3 py-2">
                        {r.status === "parsing" && (
                          <Badge variant="secondary" className="bg-slate-200 text-sm text-slate-800">
                            Reading PDF…
                          </Badge>
                        )}
                        {r.status === "ready" && (
